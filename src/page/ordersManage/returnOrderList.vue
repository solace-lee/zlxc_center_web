  ...
  <div class="fillcontain">
    <div style="width: 100%; max-height:50px;">
      ...
      >
        ...
        <div style="border-left:solid #00ae87 4px; width:82px;height:20px;margin:15px;">&nbsp;退货订单</div>
        <div style="display: flex; justify-content: space-between;margin-left: 20px; align-items:center;">
          ...
          <el-menu-item index="2" > 待收货 </el-menu-item> <div style="color:#999">/</div>  
          <el-menu-item index="9" > 完成 </el-menu-item> <div style="color:#999">/</div>  
          <el-menu-item index="3" > 拒绝 </el-menu-item> <div style="color:#999">/</div>  
          <el-menu-item index="7" > 关闭 </el-menu-item>
        </div>
        <div style="margin:6px 0px 0px 15%;" >
...
export default {
  ...
  created() {
    let returnOrderList = window.localStorage.getItem("returnOrderList");
    let orderObj = JSON.parse(returnOrderList)
    if(returnOrderList){
  ...
  beforeRouteEnter (to, from, next) {
    if(from.path != "/returnOrderDetails"){
      window.localStorage.setItem("returnOrderList","{}");
    }
    next();
  },
  mounted() {},
  methods: {
    ...
    saveLocalStorage () {
      ...
      window.localStorage.setItem("returnOrderList", JSON.stringify(returnOrderList));
    },
